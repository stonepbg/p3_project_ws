#!/usr/bin/env python3
"""
dashboard_node.py (제어 그룹 간 경계 라인 추가 버전)
──────────────────────────────────────────────────────────────────
"""
...
/* 이름별 실시간 로그 색상 스타일 세분화 정의 */
.log-cat-manager {color: #00aaff !important;}    /* [매니저] 파란색 */
.log-cat-agvmode {color: #EBB9FF !important;}    /* [agv_mode] 보라색 */
.log-cat-drive {color: #ffbe00 !important;}      /* [주행] 노란색 */
.log-cat-agvstatus {color: #ff3355 !important;}  /* [AGV_status] 빨간색 */
.log-cat-default {color: #00e5a0 !important;}    /* 기본 시스템 로그 초록색 */
...
.btn-stop, .btn-follow, .btn-action, .btn-dest, .btn-rediscover, .btn-pickup {width:100%;font-family:var(--sans);border-radius:8px;cursor:pointer;transition:all 0.15s;}

/* 정지 버튼 효과 및 스타일 */
.btn-stop{padding:8px;min-height:44px;background:transparent;border:2px solid var(--r);color:var(--r);font-size:21px;font-weight:700}
...
.btn-follow{padding:8px;min-height:44px;background:transparent;border:2px solid var(--g);color:var(--g);font-size:21px;font-weight:700}
.btn-follow:hover{background:rgba(0,229,160,0.1);border-color:#fff;box-shadow:0 0 10px rgba(0,229,160,0.3);}

.btn-pickup{padding:8px;min-height:44px;background:transparent;border:2px solid #ff7700;color:#ff7700;font-size:19px;font-weight:700}
.btn-pickup:hover{background:rgba(255,119,0,0.1);border-color:#fff;box-shadow:0 0 10px rgba(255,119,0,0.3);}

.btn-rediscover{padding:8px;min-height:44px;background:transparent;border:2px solid var(--p);color:var(--p);font-size:19px;font-weight:700}
.btn-rediscover:hover{background:rgba(176,38,255,0.1);border-color:#fff;box-shadow:0 0 10px rgba(176,38,255,0.3);}

.btn-action {padding:11px;min-height:50px;font-size:19px;font-weight:700;background:transparent;border:2px solid var(--border);color:var(--text);}
...
.btn-action#btn-auto-main:hover {background:rgba(255,190,0,0.1);border-color:#fff;}

.dest-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:6px;padding:2px;width:100%;opacity:0.3;pointer-events:none;border:2px solid transparent;border-radius:10px;transition:all 0.2s ease;}
.dest-grid.active-guide-grid {opacity:1;pointer-events:auto;border-color:rgba(0,170,255,0.2);}
.dest-grid.active-auto-grid {opacity:1;pointer-events:auto;border-color:rgba(255,190,0,0.2);}
.btn-dest{padding:11px 8px;min-height:48px;background:transparent;border:2px solid var(--bhi);color:var(--text);font-size:17px;font-weight:700}
...
.active-follow {background:var(--g) !important;color:#06080c !important;box-shadow:none !important;}
...
.active-rediscover {background:var(--p) !important;color:#fff !important;box-shadow:none !important;}
...
function applyUiState(state) {
  ...
  if (state.activeMode) {
    ...
    if (state.activeMode === 'guide') {
      ...
      guideText.classList.add('guide-theme-blue');
      ...
      guideText.classList.add('guide-theme-yellow');
...
function clickStop() { pubCmd(0, '정지'); }

function clearAllActiveStyles() {
  const allUiButtons = document.querySelectorAll('button:not(.cap-btn):not(.log-clear-btn)');
  allUiButtons.forEach(btn => btn.classList.remove('active-follow', 'active-guide', 'active-auto', 'active-rediscover', 'active-pickup'));
...
class DashboardNode(Node):
    ...
    def _cb_agv_mode_sub(self, msg):
        with self._lock:
            self._agv_log_buffer.append(f"[{datetime.now().strftime('%H:%M:%S')}] [agv_mode] param: {msg.data}")
    ...
    def _cb_agv_status(self, msg):
        with self._lock:
            self._agv_log_buffer.append(f"[{datetime.now().strftime('%H:%M:%S')}] [AGV_status] param: {msg.data}")
