#!/usr/bin/env python3
"""
dashboard_node.py (방향키 제어 추가 버전)
──────────────────────────────────────────────────────────────────
"""
...
.log-cat-agvmode {color: #EE593FF !important;}    
...
.btn-manual{padding:8px;min-height:44px;background:transparent;border:2px solid var(--navy);color:var(--navy);font-size:19px;font-weight:700}
.btn-manual:hover{background:rgba(15, 23, 42, 0.1);border-color:#fff;box-shadow:0 0 10px rgba(15, 23, 42, 0.3);}
...
.manual-grid.active-manual-grid {
  max-height: 150px;
  opacity: 1;
  padding: 6px 2px;
  margin-top: 6px;
...
.btn-man-pad strong { display: block; font-size: 11px; opacity: 0.7; }
...
.active-manual {background:var(--navy) !important;color:#ffffff !important;box-shadow:none !important;}
...
// [수정] 기존 알파벳 맵핑에 화살표 키 명칭(ArrowUp, ArrowDown, ArrowLeft, ArrowRight) 추가 통합
const keyMap = { 
  'q': 'ccw', 
...
function clearAllActiveStyles() {
  ...
  allUiButtons.forEach(btn => btn.classList.remove('active-follow', 'active-guide', 'active-auto', 'active-rediscover', 'active-pickup', 'active-manual'));
    ...
    def set_ui_mode(self, mode):
        with self._lock:
            if mode == 'manual_trigger':
                ...
                self._pub_log_buffer.append({"time": ts, "val": 60, "label": "수동 조작 모드 활성화"})
