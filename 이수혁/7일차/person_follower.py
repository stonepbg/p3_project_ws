...
class PersonFollower(Node):
    ...
    def mode_callback(self, msg):
        ...
        if self.current_agv_mode == 10 and prev_mode != 10:
            self.get_logger().info('▶️ 10번 모드 진입: 사람 추종 알고리즘을 활성화합니다.')
            # 와치독이 바로 터지지 않도록 타이머 초기화
            now = self.get_clock().now()
            self.last_msg_time = now
            self.last_log_time = now
            
        # 10번 모드에서 다른 모드로 빠져나갈 때
        elif prev_mode == 10 and self.current_agv_mode != 10:
            self.get_logger().info('⏹️ 추종 모드 해제: 추적을 중지하고 안전하게 대기합니다.')
            self.emergency_stop()

    def scan_callback(self, msg):
        # 10번 모드가 아닐 때는 라이다 연산 생략 (자원 절약)
