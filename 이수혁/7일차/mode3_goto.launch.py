...
def generate_launch_description():
    ...
    # [1단계] 주행 전 위치 초기화를 위해 기존 위치 파악 런치 파일을 먼저 실행
    ...
    # 이벤트 자동화: 위치 파악 런치 프로세스가 종료되면 주행 및 180도 회전을 위해 커맨더 시작
    start_goto_sequence = RegisterEventHandler(
        OnProcessExit(
            target_action=localization_launch_process,
            on_exit=[
                LogInfo(msg="✅ [목적지 이동 모드] vision_localization 런치가 종료되었습니다. 자율주행 커맨더를 시작합니다."),
                nav2_commander_node
            ]
        )
    )
