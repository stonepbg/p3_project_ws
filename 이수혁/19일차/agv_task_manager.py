...
class AGVTaskManager(Node):
    def __init__(self):
        ...
        self.get_logger().info('🛠️ AGV 태스크 매니저 가동 (60번 수동 조작 모드 라우팅 추가)')
    ...
    def mode_callback(self, msg):
        ...
        if new_group != old_group:
            ...
            if new_group == 1:
                ...
                # [신규] 60번 수동 조작 전용 런치 파일 실행
                self.start_process(['ros2', 'launch', 'mode6_manual.launch.py'])
            elif new_group == 0:
                self.send_log('⏹️ 대기 모드: 모든 동작을 강제 정지합니다.')

    def get_mode_group(self, mode):
        if mode in [10, 50]: return 1  
        elif 20 <= mode <= 37: return 2  
        elif mode == 40: return 4
        elif mode == 51: return 5      
        elif mode == 60: return 6      # [신규] 60번 수동 조작 모드 할당
        else: return 0
