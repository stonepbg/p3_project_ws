...
class ArucoLocalizer(Node):
    def __init__(self):
        ...
        self.get_logger().info('👁️ 시각 기반 로컬라이제이션 가동 (능동 탐색 모드 적용)')
    ...
    def control_loop(self):
        ...
        if self.state == 'SEARCHING':
            # 약 25초(1바퀴 회전) 대기 후 이동 탐색으로 전환
            if current_time - self.search_start_time > 25.0:
                self.get_logger().warn('⚠️ 한 바퀴를 돌았지만 마커가 보이지 않습니다! 사각지대 탈출을 위해 위치를 이동합니다.')
                self.state = 'SHIFTING'
            ...
            if current_time - self.shift_start_time < 3.0:
                ...
                self.get_logger().info('🔄 위치 이동 완료. 다시 주변 탐색을 시작합니다.')
                self.cmd_vel_pub.publish(Twist()) 
                self.state = 'SEARCHING'
                self.search_start_time = time.time() 
            ...
            if self.sync_count <= 15: twist.angular.z = 0.2   
                ...
                self.get_logger().info('✅ 완벽한 위치 및 각도 매칭 성공! 노드를 자동 종료합니다.')
    ...
    def image_callback(self, msg):
        if self.state not in ['SEARCHING', 'SHIFTING']:
            return
        ...
        try:
            ...
            if ids is not None:
                ...
                if detected_id in self.marker_database:
                    ...
                    self.get_logger().info(f'🎯 마커(ID: {detected_id}) 발견! 동기화 스캔 댄스를 시작합니다.')
