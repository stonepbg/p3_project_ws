...
const reverseKeyMap = { 
  'ccw': 'p-ccw', 'forward': 'p-forward', 'cw': 'p-cw', 
  'left': 'p-left', 'backward': 'p-backward', 'right': 'p-right' 
};
...
function applyUiState(state) {
  ...
  if (state.activeType) {
    ...
    if (state.activeType === 'follow') {
      ...
      document.getElementById('btn-stop-core').classList.add('active-stop');
    }
    return;
  }
...
function clearAllActiveStyles() {
  const allUiButtons = document.querySelectorAll(
    'button:not(.cap-btn):not(.log-clear-btn):not(.btn-man-pad)'
  );
  allUiButtons.forEach(btn => btn.classList.remove(
    'active-follow', 'active-guide', 'active-auto', 
    'active-rediscover', 'active-pickup', 'active-manual', 'active-stop'
  ));
...
function renderPubLogs(logList) {
  ...
  for (let i = 0; i < logList.length; i++) {
    let item = logList[i];
    html += `<div class="log-item"><span class="log-time">${item.time}</span>`;
    html += `<span class="log-msg" style="color:#ffffff">→ <span style="color:var(--y)">${item.val}</span> `;
    html += `<span style="color:#e2e8f0; font-size:14px;">(${item.label})</span></span></div>`;
  }
...
class DashboardNode(Node):
    """
    CycloneDDS 기반 AGV 원격 제어 및 멀티 스트림 데이터 처리를 위한 웹 대시보드 메인 ROS 2 노드 클래스.
    """
    ...
    def _cb_sign(self, msg): 
        """교통 표지판 이벤트 토픽 수신 버퍼 콜백 처리 인터페이스 (Dummy)"""
        pass

    def _cb_brain(self, msg): 
        """제어 코어의 메인 상태 머신 모니터링 토픽 콜백 (Dummy)"""
        pass
    
    def _cb_agv_log(self, msg):
        """하위 시스템에서 올라온 일반 로그 문자열을 타임스탬프와 함께 데크 버퍼에 저장"""
        with self._lock: 
            self._agv_log_buffer.append(f"[{datetime.now().strftime('%H:%M:%S')}] {msg.data}")

    def _cb_agv_mode_sub(self, msg):
        """AGV 상태 모드 스위칭 이벤트를 로그 버퍼에 기록"""
        with self._lock:
            self._agv_log_buffer.append(f"[{datetime.now().strftime('%H:%M:%S')}] [agv_mode] -> {msg.data}")

    def clear_agv_log_buffer(self):
        """웹 UI 요청에 의해 수신된 AGV 실시간 수신 터미널 로그 버퍼를 초기화"""
        with self._lock: 
            self._agv_log_buffer.clear()
        
    def clear_pub_log_buffer(self):
        """웹 UI 내부 대시보드 명령 패널에서 발행했던 로컬 커맨드 히스토리 백로그 전체 제거"""
        with self._lock: 
            self._pub_log_buffer.clear()

    def _cb_agv_status(self, msg):
        """AGV의 동작태스크 상태 정보를 트래킹하여 특정 예외 상황 및 UI 초기화 여부를 판단"""
        with self._lock:
            # 현재 상태가 'pickup'(픽업모드)인 경우 수신된 AGV_status 값을 무시하고 리턴합니다.
        ...
        if msg.data in [0, 1]:
            with self._lock: 
                ...
                self._current_ui_state = {
                    "activeMode": None, 
                    "activeDest": None, 
                    "activeType": "stop" if msg.data == 0 else None
                }

    def _cb_arm_status(self, msg):
        """매니퓰레이터 동작 상태(/ARM_status) 피드백 감지 시 긴급 플래그 처리 콜백"""
        if msg.data == 1:
            with self._lock:
                self._agv_log_buffer.append(
                    f"[{datetime.now().strftime('%H:%M:%S')}] [ARM_status] -> 1 (정지 명령 실행)"
                )
            self.publish_nano_command(0, "ARM_status에 의한 정지")

    def set_ui_mode(self, mode):
        """대시보드 메인 컨트롤 그룹의 글로벌 활성 주행 상태 변경 로직 제어"""
        with self._lock:
    ...
    def update_ui_state_by_cmd(self, cmd_int, label):
        """수신 및 송신 데이터에 기반하여 정수형 매핑 상태 데이터 구조를 갱신"""
        with self._lock:
            if cmd_int == 0:
    ...
    def publish_nano_command(self, cmd_int, label):
        """Jetson Nano 혹은 외부 임베디드 코어로 정수 형식 코어 태스크 타깃 인덱스 토픽 송출"""
    ...
    def publish_manual_command(self, cmd_str):
        """수동 제어 그리드에서 요구하는 문자열 형식의 방향 제어 프리셋 패킷 전달"""
        msg = String()
        msg.data = cmd_str
    ...
    def capture_single(self):
        """비동기 워커 스레드를 사용하여 현재 시점의 카메라 실시간 단일 비디오 캡처 스냅샷을 생성"""
        threading.Thread(target=self._save_snapshot, daemon=True).start()
        return True

    def _save_snapshot(self):
        """OpenCV 스트림 소스에서 프레임을 획득한 후 이미지 포맷으로 스토리지 아카이브에 바인딩"""
        try:
    ...
    def toggle_burst(self):
        """지정된 인터벌 주기 단위의 백그라운드 반복 루프 형태 멀티 스냅샷 레코딩을 반전 토글"""
    ...
    def _burst_loop(self):
        """버스트 캡처 모드가 정지 플래그로 토글될 때까지 내부 인터벌 타이머 주기에 맞추어 연쇄 스냅샷 호출"""
        while self._bursting:
            self._save_snapshot()
            time.sleep(BURST_INTERVAL)

    def toggle_video_record(self):
        """실시간 스트리밍 비디오 입력 포트 소스로부터 지속적인 MP4 비동기 스트림 아카이빙 세션을 조작"""
    ...
    def _record_video(self):
        """OpenCV 라이브러리를 바인딩하여 실시간 스트림 파이프라인 데이터 인코딩 저장을 지속 구동"""
        try:
            cap = cv2.VideoCapture(STREAM_URL_1)
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            out = cv2.VideoWriter(
                os.path.join(MOVIE_DIR, f"video_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"), 
                fourcc, 20.0, (640, 480)
            )
            ...
            self.get_logger().error(f"동영상 녹화 오류 발생: {e}")

    def get_status(self):
        """웹 프론트엔드에서 주기적으로 폴링하는 시점에 내부 로깅 큐 데이터와 실시간 상태 정보를 JSON으로 매핑"""
        with self._lock:
...
def create_app(node):
    """
    ROS 2 인스턴스 코어 핸들러 객체를 Flask 라우트 환경과 상호 결합 및 인젝션을 수행하는 팩토리 랩퍼 함수.
    """
    app = Flask(__name__)
    ...
    def api_video_toggle(): 
        return jsonify({"recording": node.toggle_video_record()})
...
def main(args=None):
    """메인 프로세스 엔트리포인트: ROS 2 환경을 활성화하고 백그라운드 플라스크 데몬 웹서버를 동시 스핀"""
    rclpy.init(args=args)
    node = DashboardNode()
    
    flask_app = create_app(node)
    threading.Thread(
